...
async function promptForStep(ctx: BotContext, state: FlowState & { flow: "add" }) {
  switch (state.step) {
    case "shopName":
    ...
    case "state":
      ...
      break;
    case "phone":
      await ctx.reply("Enter the phone number:", { reply_markup: cancelKeyboard() });
      break;
    case "contactPerson":
      await ctx.reply("Enter the contact person name:", { reply_markup: cancelKeyboard() });
      break;
    case "staffType":
    ...
    case "confirm":
      ...
      break;
    case "editField":
      await ctx.reply("What do you want to edit?", { reply_markup: editFieldsKeyboard() });
      break;
    default:
      await ctx.reply("Something went wrong. Returning to menu.");
      await clearFlowState(ctx.kv, requireChatUser(ctx).chatId, requireChatUser(ctx).userId);
...
export async function startAddShop(ctx: BotContext): Promise<void> {
  ...
  await ctx.reply("Add a new shop (wizard). You can cancel anytime.", { reply_markup: cancelKeyboard() });
...
export async function handleAddShopText(
  ...
  switch (state.step) {
    case "shopName":
      if (!t) {
        await ctx.reply("Shop name cannot be empty. Try again:", { reply_markup: cancelKeyboard() });
        return true;
      }
    ...
    case "address":
      if (!t) {
        await ctx.reply("Address cannot be empty. Try again:", { reply_markup: cancelKeyboard() });
        return true;
      }
    ...
    case "city":
      if (!t) {
        await ctx.reply("City cannot be empty. Try again:", { reply_markup: cancelKeyboard() });
        return true;
      }
      state.data.city = t;
      state.step = "state";
    ...
    case "phone":
      if (!t) {
        await ctx.reply("Phone cannot be empty. Try again:", { reply_markup: cancelKeyboard() });
        return true;
      }
      state.data.phone = t;
    ...
    case "contactPerson":
      if (!t) {
        await ctx.reply("Contact person cannot be empty. Try again:", { reply_markup: cancelKeyboard() });
        return true;
      }
...
async function finalizeAndSave(ctx: BotContext, state: FlowState & { flow: "add" }) {
  ...
  if (state.servicesSelected.length === 0) {
    // Services are "multi-select", but not explicitly marked required; still nudge.
    await ctx.reply("No services selected. If that's correct, tap Save again. Or Edit to add services.");
    return;
  }
...
export async function handleAddShopCallback(
  ...
  if (data.startsWith("add:toggle_service:")) {
    ...
    await setFlowState(ctx.kv, chatId, userId, state);
    await ctx.answerCallbackQuery();
    // edit the message to update keyboard (best-effort)
    try {
      await ctx.editMessageReplyMarkup({ reply_markup: servicesKeyboard(state.servicesSelected) });
    } catch {
